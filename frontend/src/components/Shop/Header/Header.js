...
const header = () => {
  ...
  const { carts, user } = useSelector(headerSelector);
  
  const showBadge = router.pathname === '/cart' || router.pathname === '/checkout'  ? false : true;
  ...
  let cartButttonIcon = (
    <Link href="/cart" color="inherit">
      <IconButton aria-label="shop icon" edge="end" color="inherit" className={classes.shopingCart}>
        <StyledBadge badgeContent={showBadge ? carts.length : 0} color="secondary">
